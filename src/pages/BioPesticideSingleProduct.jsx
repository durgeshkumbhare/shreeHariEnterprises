...
const BioPesticideSingleProduct = () => {
  const { productId } = useParams();
  const product = pesticides.find((product) => product.id == productId);
  const { img, model, description, reccomendedCrops, packaging } = product;
  return (
    <div className="m-8">
      <h1 className="font-bold text-2xl">{model}</h1>
      <div className="grid md:grid-cols-2 max-w-[60rem] mx-auto mt-8 gap-4 border-b-2 pb-6 border-black">
      ...
      <div>
        ...
        <p className="mb-2">
          <span className="font-bold">Phone: </span>7517043603
        </p>
        <p className="mb-2">
          <span className="font-bold">Email:</span>{" "}
          shreeharienterprises457@gmail.com
        </p>
        <p className="mb-2">
          <span className="font-bold">WhatsApp:</span> 7517043603
        </p>
        <p>
          Alternatively, you can send us a direct message, and our team will get
          back to you promptly. Let’s help your fields flourish!
        </p>
      </div>
    </div>
  );
...
export default BioPesticideSingleProduct;
